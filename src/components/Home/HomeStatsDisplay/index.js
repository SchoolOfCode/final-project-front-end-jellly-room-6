...
import {
  getUserRemainingXpPercentage,
  getUserXpNextLevelRequired,
  getUserXpEarnedThisLevel,
...
export default function HomeStatsDisplay({ userInfo }) {
  ...
  return (
    <div className={`${styles.statsDisplay}`}>
      <p className={styles.text}>{"Pick a topic, take the quiz and earn some beans. Let's go!"}</p>
      ...
      <div className={styles.levelBarContainer}>
        ...
        <div className={styles.levelBarNextLevel}>
          <p>{userInfo.playerLevel + 1}</p>
        </div>
      </div>

      <p>
        {getUserXpEarnedThisLevel(userInfo.xp, userInfo.playerLevel)}/
        {getUserXpNextLevelRequired(userInfo.playerLevel)} XP
      </p>
    </div>
  );
}
